...
#
# Alongside it the folder needs compose.yaml from this repo, and a .env holding
# the settings for this machine. It is not red/green on purpose: the editor holds
# no session, and two containers writing the same data folder is the one thing
...
if [[ ! -f compose.yaml ]]; then
  echo "compose.yaml is missing from $HERE" >&2
  exit 1
fi
