/* The selector, lifted verbatim from reference/artwork/OSC. 1 (1).svg: the tick
   marks, the six detents and where their labels sit, and the knob itself. Only
   the grouping and the colour references are ours — do not redraw them.

   It is its own knob and not the KNOB 5 every continuous control turns. The
   instrument's rotary selector is a different part — a pointer on a wide fluted
   skirt against KNOB 5's dot on a narrow ring, as reference/measurements.md
   records it — and drawing both from one export made the panel agree with
   itself while disagreeing with the instrument.

   The export is drawn with the knob turned to −15°, the fourth detent, which is
   what BAKED_ANGLE is for: rendering a position rotates the body group by the
   difference and nothing here is re-pathed.
